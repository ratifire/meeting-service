...
function handleRemovePeer(config) {
  ...
  // Check if the removed peer was Bot Recorder and hide recording indicator
  if (allPeers[peer_id] && allPeers[peer_id]["peer_name"] === "Bot Recorder") {
  ...
  // Check if there are any other Bot Recorder participants still in the meeting
  const hasBotRecorder = Object.values(allPeers).some(
    (peer) => peer.peer_name === "Bot Recorder",
  );
  if (!hasBotRecorder) {
    hideRecordingIndicator();
  }
...
async function loadRemoteMediaStream(stream, peers, peer_id, kind) {
  ...
  switch (kind) {
    case "video":
      ...
      // Check if this is Bot Recorder and hide it
      if (peer_name === "Bot Recorder") {
...
async function snapshotRoom() {
  ...
  try {
    ...
    video.onplay = async () => {
      ...
      link.download =
        "Room_" + roomId + "_" + getDataTimeString() + "_snapshot.png";
      link.click();
...
async function refreshMyStreamToPeers(stream, localAudioTrackChange = false) {
  ...
  for (const peer_id in peerConnections) {
    ...
    // Replace video track
    const videoSender = peerConnections[peer_id]
      .getSenders()
    ...
    // Replace audio track
    const audioSender = peerConnections[peer_id]
      .getSenders()
...
function appendMessage(from, img, side, msg, privateMsg, msgId = null) {
  ...
  const getImg =
    isChatGPTOn && getSide === "left" ? images.chatgpt : filterXSS(img);
  ...
  const isValidPrivateMessage =
    getPrivateMsg && getMsgId != null && getMsgId != myPeerId;
...
function speechMessage(newMsg = true, from, msg) {
  ...
  speech.text =
    (newMsg ? "New" : "") + " message from:" + from + ". The message is:" + msg;
  speech.rate = 0.9;
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          if (!recognitionRunning) {
            const { recognitionLanguageIndex, recognitionDialectIndex } =
              config.data;
...
/**
 * Show recording indicator dot in top-right corner
 */
function showRecordingIndicator() {
  if (getId("recordingIndicator")) return; // Already exists
  ...
  indicator.title = "Meeting is being recorded";

  document.body.appendChild(indicator);
}

/**
 * Hide recording indicator dot
 */
function hideRecordingIndicator() {
  const indicator = getId("recordingIndicator");
  if (indicator) {
    indicator.remove();
  }
}
