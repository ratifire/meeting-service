...
const hostUsersString =
  process.env.HOST_USERS || '[{"username": "MiroTalk", "password": "P2P"}]';
...
const surveyURL =
  process.env.SURVEY_URL || "https://skillzzy.com/interviews/scheduled";
...
app.post(["/login"], (req, res) => {
  ...
  if (isPeerValid) {
    ...
    const isPresenter =
      roomPresenters && roomPresenters.includes(username).toString();
    const token = encodeToken({
      username: username,
      password: password,
...
app.get("/buttons", (req, res) => {
  res
    .status(200)
    .json({ message: config && config.buttons ? config.buttons : false });
});
...
app.get("/brand", (req, res) => {
  res
    .status(200)
    .json({ message: config && config.brand ? config.brand : false });
});
  ...
  socket.on("data", async (dataObj, cb) => {
    ...
    switch (method) {
      ...
      case "getChatGPT":
        ...
        if (!configChatGPT.enabled) {
          return cb({ message: "ChatGPT seems disabled, try later!" });
        }
...
function isPeerPresenter(room_id, peer_id, peer_name, peer_uuid) {
  try {
    if (!presenters[room_id] || !presenters[room_id][peer_id]) {
      ...
      for (const [existingPeerID, presenter] of Object.entries(
        presenters[room_id] || {},
      )) {
