...
if (turnServerEnabled && turnServerUrl && turnServerUsername
...
const ipWhitelist = {
  ...
  allowed: process.env.IP_WHITELIST_ALLOWED ? JSON.parse(
      process.env.IP_WHITELIST_ALLOWED) : [],
};

// OIDC - Open ID Connect
const OIDC = {
  enabled: process.env.OIDC_ENABLED ? getEnvBoolean(process.env.OIDC_ENABLED)
      : false,
  config: {
    ...
    authRequired: process.env.OIDC_AUTH_REQUIRED ? getEnvBoolean(
        process.env.OIDC_AUTH_REQUIRED) : false, // Set to true if authentication is required for all routes
...
// stats configuration
const statsData = {
  enabled: process.env.STATS_ENABLED ? getEnvBoolean(process.env.STATS_ENABLED)
      : true,
...
app.use(apiBasePath + '/docs', swaggerUi.serve,
    swaggerUi.setup(swaggerDocument)); // api docs
...
app.get('/join/', async (req, res) => {
  if (Object.keys(req.query).length > 0) {
    ...
    */
    const {room, name, audio, video, screen, notify, hide, token} = checkXSS(
        req.query);
    ...
    const allowRoomAccess = isAllowedRoomAccess('/join/params', req, hostCfg,
        peers, room);
    ...
    if (token) {
      try {
        ...
        return hostCfg.protected || hostCfg.user_auth ? res.sendFile(
            views.login) : res.sendFile(views.landing);
      }
    }
    ...
    if ((hostCfg.protected && isPeerValid && isPeerPresenter
...
app.get('/join/:roomId', function (req, res) {
  ...
  const allowRoomAccess = isAllowedRoomAccess('/join/:roomId', req, hostCfg,
      peers, roomId);
...
app.post(['/login'], (req, res) => {
  ...
  if (hostCfg.protected && isPeerValid && !hostCfg.authenticated) {
    ...
    const token = encodeToken(
        {username: username, password: password, presenter: true});
  ...
  if (isPeerValid) {
    ...
    const isPresenter = roomPresenters && roomPresenters.includes(
        username).toString();
    const token = encodeToken(
        {username: username, password: password, presenter: isPresenter});
    return res.status(200).json({message: token});
  } else {
...
app.post('/slack', (req, res) => {
  ...
  if (!slackSigningSecret) {
    return res.end('`Slack Signing Secret is empty!`');
  }
  ...
  if (Math.abs(time - timeStamp) > 300) {
    return res.end(
        '`Wrong timestamp` - Ignore this request.');
  }

  // Get Signature to compare it later
  const sigBaseString = 'v0:' + timeStamp + ':' + requestBody;
  const mySignature = 'v0=' + CryptoJS.HmacSHA256(sigBaseString,
      slackSigningSecret);
...
io.sockets.on('connect', async (socket) => {
  ...
  socket.conn.on('upgrade', () => {
    ...
    log.debug('[' + socket.id + '] Connection upgraded transport',
        upgradedTransport);
  });
    ...
    switch (method) {
      case 'checkPeerName':
        log.debug('Check if peer name exists',
            {peer_name: peer_name, room_id: room_id});
  ...
  socket.on('join', async (cfg) => {
    ...
    if (hostCfg.user_auth || peer_token) {
      ...
      if (peer_token) {
        try {
          ...
          is_presenter =
              presenter === '1' || presenter === 'true' || Object.keys(
                  presenters[channel]).length === 0;
          ...
          // redirect peer to login page
          log.error('[' + socket.id + '] [Warning] Join Room JWT error',
              err.message);
          return socket.emit('unauthorized');
        }
      } else {
    ...
    if (peers[channel]['lock'] === true && peers[channel]['password']
        != channel_password) {
    ...
    const isPresenter = peer_token ? is_presenter : isPeerPresenter(channel,
        socket.id, peer_name, peer_uuid);
  ...
  socket.on('roomAction', async (cfg) => {
    ...
    log.debug('[' + socket.id + '] Room ' + room_id,
        {locked: room_is_locked, password: password});
  });
  ...
  socket.on('peerName', async (cfg) => {
    ...
    for (let peer_id in peers[room_id]) {
      if (peers[room_id][peer_id]['peer_name'] == peer_name_old && peer_id
          == socket.id) {
  ...
  socket.on('peerStatus', async (cfg) => {
    ...
    try {
      for (let peer_id in peers[room_id]) {
        if (peers[room_id][peer_id]['peer_name'] == peer_name && peer_id
            == socket.id) {
          switch (element) {
            case 'video':
  ...
  socket.on('peerAction', async (cfg) => {
    ...
    if (presenterActions.some((v) => peer_action === v)) {
      // Check if peer is presenter
      const isPresenter = isPeerPresenter(room_id, peer_id, peer_name,
          peer_uuid);
    ...
    if (send_to_all) {
      ...
      log.debug('[' + socket.id + '] emit peerAction to [' + peer_id
  ...
  socket.on('kickOut', async (cfg) => {
    ...
    // Check if peer is presenter
    const isPresenter = await isPeerPresenter(room_id, peer_id, peer_name,
        peer_uuid);

    // Only the presenter can kickOut others
    if (isPresenter) {
      log.debug(
          '[' + socket.id + '] kick out peer [' + peer_id + '] from room_id ['
          + room_id + ']');
  ...
  socket.on('fileInfo', async (cfg) => {
    ...
    log.debug(
        '[' + socket.id + '] Peer [' + peer_name + '] send file to room_id ['
        + room_id + ']', {
          peerName: peer_name,
  ...
  socket.on('fileAbort', async (cfg) => {
    ...
    log.debug('[' + socket.id + '] Peer [' + peer_name
  ...
  socket.on('videoPlayer', async (cfg) => {
    ...
    if (peer_id) {
      log.debug('[' + socket.id + '] emit videoPlayer to [' + peer_id
          + '] from room_id [' + room_id + ']', data);
  ...
  async function removePeerFrom(channel) {
    ...
    log.info('[removePeerFrom] - connected presenters grp by roomId',
        presenters);
...
function getEnvBoolean(key, force_true_if_undefined = false) {
  if (key == undefined && force_true_if_undefined) {
    return true;
  }
...
function isPeerPresenter(room_id, peer_id, peer_name, peer_uuid) {
  try {
    if (!presenters[room_id] || !presenters[room_id][peer_id]) {
      ...
      for (const [existingPeerID, presenter] of
          Object.entries(presenters[room_id] || {})) {
        if (presenter.peer_name === peer_name) {
          log.debug('[' + peer_id + '] Presenter found',
...
function isAuthPeer(username, password) {
  return hostCfg.users && hostCfg.users.some(
...
function encodeToken(token) {
  ...
  const encryptedPayload = CryptoJS.AES.encrypt(payloadString,
      jwtCfg.JWT_KEY).toString();
...
function decodeToken(jwtToken) {
  ...
  const decryptedPayload = CryptoJS.AES.decrypt(decodedToken.data,
      jwtCfg.JWT_KEY).toString(CryptoJS.enc.Utf8);
...
function allowedIP(ip) {
  ...
  log.info('Allowed IPs',
      {ip: ip, authorizedIP: authorizedIP, authorizedIPs: authorizedIPs});
