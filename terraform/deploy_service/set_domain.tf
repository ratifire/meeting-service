resource "aws_lb_listener" "https_ecs_meeting_listener" {
  load_balancer_arn = aws_lb.meeting_ecs_alb.arn
  port              = 443
  protocol          = "HTTPS"
...
resource "aws_route53_record" "sub_a_record" {
  ...
  type    = "A"
  alias {
    name                   = aws_lb.meeting_ecs_alb.dns_name
    zone_id                = aws_lb.meeting_ecs_alb.zone_id
